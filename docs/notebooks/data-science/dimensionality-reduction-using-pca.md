...
![Projecting Down from 3D to 2D PCA](../../assets/images/notebooks/dimentionality-reduction-pca-gif.gif)
...
![From 3D to 2D PCA](../../assets/images/notebooks/dimensionality-reduction-projecting-down.png)
...
The explained variance ratio indicates the proportion of the dataset's variance that lies along the axis of each principal component. Following the iris example, let's see the PCs variance.
...
[Minimum dimensions required to preserve 95% of the variance.](../../assets/images/notebooks/dimensionality-reduction-right-dimensions.png)
...
It is also possible to decompress the reduced dataset back to 1,2 million features by applying the inverse transformation of the PCA projection. Of course this wont give you back the original data, since the projection lost a bit of information, but it will likely be quite close to the original data. The mean squared distance between the original data and the reconstructed data is called reconstruction error.
...
def view_pca(array, pcs=4):
    fig = plt.figure(figsize=(20, 200))
    grid = ImageGrid(fig, 111, nrows_ncols=(1, pcs), axes_pad=0.1)
    imgs = []
    for i in range(pcs):
        img = np.array(array[i]).reshape(1200, 1000)
        imgs.append(img)
    for ax, im in zip(grid, imgs):
        ax.imshow(im, cmap='gray')
    plt.show()
...
### 01. Loading the pictures dataset
...
![Original pictures of myself](../../assets/images/notebooks/dimensionality-reduction-original-pics.png)
...
![Visualizing eigenvectors](../../assets/images/notebooks/dimensionality-reduction-eigenvectors.png)
...
![Visualizing the reconstructed images](../../assets/images/notebooks/dimensionality-reduction-reconstructed-pics.png)
...
![Visualizing the lost of variance](../../assets/images/notebooks/dimensionality-reduction-lost-variance-pics.png)
